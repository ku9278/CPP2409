...
// 사용자 정의 함수
vector<vector<int>> ReadMap(string dir);
...
int main() {
	...
	while(1){
		int class_num;
		cout << "직업을 선택하십시오(1: magician, 2: warrior): ";
		cin >> class_num;
...
vector<vector<int>> ReadMap(string dir){
	...
	// txt 파일에 저장된 맵의 크기가 코드에서 지정된 맵의 크기와 맞지 않을경우 예외를 발생시킨다.
	if(line.size() != 0 || map.size() != mapY){
		throw runtime_error("맵이 정해진 범위를 초과하였습니다");
	}
